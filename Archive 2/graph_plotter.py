...
class GraphPlotter:
    ...
    def __init__(self, ticker_name: str, plot_df: pd.DataFrame, metrics:str =""):
    ...
    def plot(self) -> None:
        ...
        #If we have metrics, we can add them as a subtitle
        sub_title = f"{self._ticker_name} Stock Price & Trend Analysis"
        
        if self._metrics:
            sub_title += f"\n{self._metrics}"

        #title
        plt.title(f"{sub_title} Stock Price & Trend")
        plt.legend()
