...
class DataBaseServer:
    ...
    def __init__(self, ticker, db_name):
        #only storing config, not actual date to ensure the classes modularity
        self._ticker = ticker
        self._db_name = db_name
    

    def save_to_table(self, data_frame) -> str | None:
        try:
            #context manager to ensure proper connection closure
            with sqlite3.connect(self._db_name, timeout=10) as conn:
                #timeout=10 was added to ensure write safety
